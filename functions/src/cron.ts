...
export const deleteOldLobbies = functions.pubsub.schedule("15 3 * * *").onRun(async (context) => {
  const oneDayAgoAsSecs = Timestamp.now().seconds - 24 * 60 * 60; // A day ago from today in seconds
  const oneDayAgoAsDate = Timestamp.fromDate(new Date(oneDayAgoAsSecs * 1000)); // a day ago from today in date object
